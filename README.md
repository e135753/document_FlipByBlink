電子書籍まばたきUI_研究資料まとめ
============================
ある学部生の現在進行中の卒業研究の公開ページ。卒論のweb版的な感じ。

![イメージ図](flow.png)

Abstract
--------
電子書籍を取り巻く状況は多くの観点で未だに成熟しているとは言い難い。PDF や WEB ベースの UI を転用し最低限の快適さをもった UI は浸透したが、電子書籍に適した UI の模索の余地はまだある。

**閉瞼を継続させた随意性瞬目をベースとした電子書籍のページめくりUIの提案。**画像に示すように閉瞼を少し長めに行うとページ送りを行い、目を開けた際には次のページに遷移しているUI。

比較的長時間の瞬目は日常的な動作である故に自然かつ快適に扱えることを期待。普及している一般的なUIに追加される形。障害者向けというわけではなく、広く一般的なUIとして。

提案する手法は「固定レイアウト」の電子書籍のみを対象。例えば紙本をそのまま転用したものや漫画。

本ドキュメントの目次
=================
- 研究の進捗状況
- デモアプリ作ったよ
- 研究テキストを全部載せるよ(工事中)
- もうすぐやる予定の実験について
- その他、雑多

今後の予定
========
☑︎ デモアプリの作成

☑︎ デモアプリをAppstoreで公開

☐ 実験概要作成

☐ デモアプリを用いて実験

☐ 論文執筆

☐ 卒研最終発表スライド作成

☐ 190213 予稿PDF提出

☐ 190220 卒研最終発表

☐ 190221 卒論PDF提出

デモアプリ
========
[AppStoreでデモアプリを公開。](https://itunes.apple.com/jp/app/flipbyblink-まばたき読書/id1444571751?mt=8)

FaceID搭載のiOS端末でのみ動作します。iPhone XやiPad pro(2018)など。

[ソースコードはGitHubで公開。](https://github.com/e135753/app_FlipByBlink)

本ドキュメントとは別リポジトリとしてOSS的なアレとして。

その他、雑多
==========

本ドキュメントについて
------------------

本ドキュメントはGitHubで管理。

見やすい静的サイトとしてGitHub Pagesで以下のURLで公開。

https://e135753.github.io/document_FlipByBlink/

このドキュメントは卒論完成したら凍結して恒久的に公開予定。OSS的にMITっぽいアレな感じで。

何か指摘、感想、アドバイスあれば

[本ドキュメントのgithubのissues](https://github.com/e135753/document_FlipByBlink/issues)

にどうぞ。

中間予稿
-------
[中間予稿のPDF](中間予稿.pdf)
